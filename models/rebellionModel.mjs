...
export class RebellionModel extends foundry.abstract.TypeDataModel {
	static defineSchema() {
		...
		return {
			details: new fields.SchemaField({
				rank: new fields.NumberField({ integer: true, min: 1, max: 20, initial: 1, nullable: false }),
				maxRank: new fields.NumberField({ integer: true, min: 5, max: 20, initial: 5, nullable: false }),
				focus: new fields.StringField({ blank: true, choices: ['loyalty', 'secrecy', 'security']}),
				membership: new fields.NumberField({ integer: true, min: 0, initial: 0, nullable: false }),
				supporters: new fields.NumberField({ integer: true, min: 0, initial: 0, nullable: false }),
				population: new fields.NumberField({ integer: true, min: 0, initial: 11900, nullable: false }),
				treasury: new fields.NumberField({ integer: true, initial: 10, nullable: false }),
				notoriety: new fields.NumberField({ integer: true, min: 0, max: 100, initial: 0, nullable: false }),
				danger: new fields.NumberField({ integer: true, initial: 20, nullable: false })
			}),
			officers: new fields.SchemaField({ // TODO make link to actors instead
	...
	prepareDerivedData() {
		...
		this.eventChance = Math.clamped((this.details.notoriety + this.details.danger) * (this.doubleEventChance ? 2 : 1), 10, 95)
	}
}